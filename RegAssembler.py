...
def rename_input(inp1, inp2):
    ...
    reads=[]
    number=-1
    with open('rawReads.fasta','w') as fout:
...
if __name__ == "__main__":
    ...
    os.system("makeblastdb -in rawReads.fasta -dbtype nucl -out db > /dev/null 2>&1")
    if os.path.exists("blastnResults"):
        os.system("rm -rf blastnResults")
    os.system("mkdir blastnResults")
    print('\nRunning Blastn...')
    os.system("cat rawReads.fasta | parallel --recstart '>' -N %d --pipe blastn -evalue 0.01 -gapopen 5 -gapextend 2 -penalty -2 -reward 1 -word_size 10 -perc_identity %d -outfmt 6 -db db -query - -out 'blastnResults/blastnResult-{#}'"%(recordNum, args.ip))
